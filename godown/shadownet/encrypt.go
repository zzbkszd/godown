...
/**
来自于github.com/shadowocks/shadowsocks-go 的加密算法实现
*/
import (
	"crypto/aes"
	"crypto/cipher"
...
var leakyBuf = shadowsocks.NewLeakyBuf(2048, leakyBufSize)
...
// 根据python shadowsocks的实现习惯实现的加密类
// 会在第一次传输的时候将iv作为前缀进行传输
func NewSSCipher(method, password string) (*SSCipher, error) {
	sc := SSCipher{}
	var err error
	sc.cipher, err = NewCipher(method, password)
	sc.send_iv = false
	return &sc, err
}

func (c *SSCipher) encrypt(src []byte) []byte {
	if directConn {
		return src
	}
	if c.cipher.enc == nil {
		iv, _ := c.cipher.initEncrypt()
		c.iv = iv
	}
	...
	if c.send_iv {
		...
		iv_dst := make([]byte, len(c.iv))
		copy(iv_dst, c.iv)
		iv_dst = append(iv_dst, dst...)
		c.send_iv = true
		return iv_dst
	}
}
func (c *SSCipher) decrypt(src []byte) (dst []byte, cut int) {
	if directConn {
		return src, 0
	}
	cut = 0
	//dp("iv:", hex.EncodeToString(c.remote_iv))
	//dp("src:", hex.EncodeToString(src))
	if c.cipher.dec == nil {
		dp(fmt.Sprintf("init decryptor, iv len: %d", c.cipher.info.ivLen))
		iv := src[:c.cipher.info.ivLen]
		c.remote_iv = iv
		c.cipher.initDecrypt(iv)
		src = src[c.cipher.info.ivLen:]
		cut = len(c.iv)
	}
	dst = make([]byte, len(src))
